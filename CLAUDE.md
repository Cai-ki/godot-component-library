...
Godot 4.6 UI 组件库，包含 **17 个**独立可复用组件 + **16 页**交互式展示应用。
...
**Overlay 组件**（UIToast / UITooltip / UIContextMenu）共用同一套架构：
- 继承 `Node`（不是 Control，无视觉节点）
- 在 `get_tree().root` 按需创建具名 `CanvasLayer`，层级依次为 100 / 101 / 102
...
7. **Array 元素类型推断失败**: `var f := array[i]` 在 Variant 数组中会报错，改用 `var f: Array = array[i]` 显式声明类型
