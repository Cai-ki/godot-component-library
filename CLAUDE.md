...
Godot 4.6 UI 组件库，包含 **24 个**独立可复用组件 + **15 页**交互式展示应用。
...
**Overlay 组件**（UIToast / UITooltip / UIContextMenu / UISelect）共用同一套架构：
- 继承 `Node`（不是 Control，无视觉节点）或 `VBoxContainer`（UISelect）
- 在 `get_tree().root` 按需创建具名 `CanvasLayer`，层级依次为 100 / 101 / 102 / 103
