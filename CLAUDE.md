...
Godot 4.6 UI 组件库，包含 **28 个**独立可复用组件 + **18 页**交互式展示应用。
...
scripts/
  ...
  pages/                      <- 18 个展示页面（每页一个 class_name, extends RefCounted）
components/                   <- 28 个独立组件（每个组件一个子目录）
```
...
**Overlay 组件**（UIToast / UITooltip / UIContextMenu / UISelect / UIDrawer）共用同一套架构：
- 继承 `Node`（不是 Control，无视觉节点）或 `VBoxContainer`（UISelect）
- 在 `get_tree().root` 按需创建具名 `CanvasLayer`，层级依次为 100 / 101 / 102 / 103 / 104
